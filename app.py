...
with st.expander("➕ REGISTRA NUOVA VISITA", expanded=False):
    # Riproduzione file audio crm.mp3
    if os.path.exists("crm.mp3"):
        st.audio("crm.mp3", format="audio/mp3", autoplay=True)
        ...
        if st.button("⚠️ AVVIA RIPRISTINO", type="primary", use_container_width=True):
            try:
                ...
                if 'cliente' in df_ripristino.columns:
                    ...
                    st.success("✅ Ripristinato! Riavvio..."); time.sleep(2); st.rerun()
